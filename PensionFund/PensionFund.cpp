...
public:
	...
	Fund(string city, float capital = 1000000, float crisis = 1.0, float defaultPension = 11000) {
		this->city = city;
		this->capital = capital;
		this->crisis = crisis;
		this->defaultPension = defaultPension;
	...
	void changeCrisis(float crisischange) {
		this->crisis += crisischange;
	}
};
...
void addFundsRational(Processes* processes, Government* government, PensionMinistry* pensionministry, Fund* fund, vector<Fund*> *funds);

int main()
{
	srand(time(0));
	//01.01.2019
	//name,sex,yyyy,mm,degree,vet,labor,salary
	...
	PensionMinistry pensionMinistry = PensionMinistry(funds);
	Government government = Government(funds);
	Processes processes = Processes(funds);

	Fund fund5("KZN", 2000000, 1.07);

	addFundsRational(&processes, &government, &pensionMinistry, &fund5, &funds);
...
void addFundsRational(Processes* processes, Government* government, PensionMinistry* pensionministry, Fund* fund, vector<Fund*> *funds) {
	processes->addFund(fund);
	government->addFund(fund);
	pensionministry->addFund(fund);
